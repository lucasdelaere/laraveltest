    ...
    <div class="row mt-3">
        <div class="col-lg-4 offset-lg-4 shadow-lg p-3 mb-5 bg-white rounded">
            ...
            <form method="POST" action="{{ action('App\Http\Controllers\ContactController@store') }}">
                @csrf <!-- belangrijk in elke form! (al ingebakken bij laravel collective -->
                @method('POST') <!-- 'POST' / 'DELETE' / 'PATCH' (update) -->
                ...
                <div class="form-floating mb-3">
                    ...
                    <label for="floatingInputValue2">name@example.com</label>
                    @error('email')
                    <p class="text-danger fs-6">{{$message}}</p>
                    @enderror
                </div>
